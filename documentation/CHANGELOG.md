...
* An invisible player's death, kill and leave messages are now obfuscated. Somebody nobody can see has already left the shot, and naming them in chat put them back in it. The announcement itself stays readable — it is still plainly a death, a kill or a disconnection — and only the name inside it is scrambled. `invisible-obfuscation` in `death.yml` ships as `names`; `message` scrambles the whole announcement instead, and `off` restores the ordinary messages. Both the account name and an active nickname are covered, and the hover card and shift-click insertion vanilla attaches to a name are cleared as well, so the identity cannot be read back out of a message it was scrambled in. Invisibility means the potion effect or the invisible player flag.
