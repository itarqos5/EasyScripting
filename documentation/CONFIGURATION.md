...
Current schema/reference: **EasyScripting 0.2.5**. See [the shipped YAML files](../src/main/resources/) for the authoritative defaults and inline explanations.

Files are generated under `plugins/EasyScripting/`. Keep your existing files when updating. Missing top-level files are copied from the JAR. Existing values are preserved except for the documented, backed-up GUI/default migrations below; exact old shipped message/help text is updated in memory. `nicknames.yml` controls the temporary-nickname lookup: the username API and its timeout, the local fallback, and `random-skin`, which also disguises a nicknamed player with a random public skin. Reload uses `/es reload`; definitions edited outside the plugin load during a server restart. A file that cannot be parsed or fails validation is isolated: the full error is logged to console, that one file falls back to the copy bundled in the jar, and every other file loads normally. The broken file on disk is never overwritten or replaced, so you keep everything you wrote — correct it and run `/es reload`. While a file is on jar defaults, operators are told which file it is when they join, and any command that would save that file is refused rather than writing the defaults over it. Shipped explanations are added to uncommented existing keys on reload; configured values and your own comments stay intact. The asynchronous YAML writer preserves headers, nested comments and inline comments.
...
See the fully commented `actor-ai.yml` and [NPC guide](NPC-GROUPS.md). `movement` controls eye tracking, social radius, wander/home radius, walking speed and wander cadence. `groups` controls enablement, group/target limits, shared path budget, general/follow repathing, formation goal change/arrival distance, the settling margin before a parked member walks again, row spacing, the number of members per row, how far a falling NPC is left to gravity, normal follow speed, catch-up distance/speed, intermediate waypoint distance, chase speed, engagement/reach and attack/knockback timing. `combat` controls carried-totem handling, reaction delays, close-range and edge-of-reach accuracy, attack jitter, waiting for the held weapon to recharge, critical-hit hops, jumps, beneficial potion bursts, and delayed shield use against a mace overhead, whether held or merely carried. `survival` controls the health thresholds and cooldowns for breaking off to eat a carried golden apple and for throwing a carried ender pearl, how far and how long an NPC backs off, and how often it circles its target between swings.
