...
* Drink an invisibility potion, then kill somebody and die yourself. Both messages must still read as a kill and a death, with only your own name scrambled inside them; the other player's name and the rest of the sentence stay readable, and hovering and shift-clicking the scrambled name must give nothing away. Repeat with `invisible-obfuscation: message` and confirm the whole announcement scrambles instead.
